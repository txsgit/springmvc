...
(function (b) {
/*
    b.$github_jquery_form_url = "http://malsup.github.io/jquery.form.js";
*/
    b.$github_jquery_form_url = kttxRoot+"/resources/common/js/jquery.form.js";
    if (b.fn.ajaxForm == undefined) {
        //b.getScript(b.$github_jquery_form_url)
    }
    var a = {};
    ...
    b.fn.uploadFile = function (r) {
        ...
        var q = b.extend({
            ...
            dragDropStr: "<span><b>拖拽文件</b></span>",
            abortStr: "中止",
            cancelStr: "取消",
            deletelStr: "删除",
            doneStr: "完成",
            multiDragErrorStr: "不允许多文件拖拽。",
            extErrorStr: "不允许的文件类型。允许的文件类型： ",
            sizeErrorStr: "该文件太大。 文件不大于：",
            uploadErrorStr: "文件上传失败。"
        }, r);

        if (b.fn.ajaxForm == undefined) {
            if (q.jquery_form_url == undefined || q.jquery_form_url == ''){
                q.jquery_form_url = b.$github_jquery_form_url;
            }
            b.getScript(q.jquery_form_url);
        }
        ...
        function f(v, t, u) {
            ...
            u.on("drop", function (w) {
                ...
                if (!t.multiple && s.length > 1) {
                    if (t.showError) {
                    	uk.info.fail(t.multiDragErrorStr);
                    }
                    return
                }
        ...
        function k(G, A, t) {
            for (var B = 0; B < t.length; B++) {
                if (!c(A, G, t[B].name)) {
                    if (G.showError) {
                    	uk.info.fail(t[B].name + G.extErrorStr + G.allowedTypes);// + A.errorLog);
                        //b("uk.info.fail(" + t[B].name + G.extErrorStr + G.allowedTypes + ")").appendTo(A.errorLog)
                    }
                    continue
                }
                if (G.maxFileSize != -1 && t[B].size > G.maxFileSize) {
                    if (G.showError) {
                    	uk.info.fail(t[B].name + G.sizeErrorStr + h(G.maxFileSize));// + A.errorLog);
                        //b("uk.info.fail(" + t[B].name + G.sizeErrorStr + h(G.maxFileSize) + ")").appendTo(A.errorLog)
                    }
                    continue
                }
                var C = G;
        ...
        function p(x, A, C, t) {
            ...
            y.change(function () {
                ...
                if (this.files) {
                    ...
                    if (!c(x, C, H)) {
                        if (C.showError) {
                            uk.info.fail(H + C.extErrorStr + C.allowedTypes);
                        }
                        return
                    }
        ...
        function o(u, t) {
            this.statusbar = b("<div class='ajax-file-upload-statusbar none'></div>");//添加“none” 因为上传图片时出现了1秒闪现图片路径的字样
        ...
        function m(y, x, t, v, z) {
            ...
            var u = {
                ...
                beforeSubmit: function (E, B, D) {
                    ...
                    uk.info.fail(x.uploadErrorStr);
                    //t.statusbar.append("uk.info.fail(" + x.uploadErrorStr + ")");
                    t.cancel.show();
                    y.remove();
